...
<template>
  <div class="relative min-h-screen w-full flex items-center justify-center">
    <button
      @click="router.back()"
      class="absolute top-6 left-6 flex items-center gap-2 text-white hover:opacity-80 transition-opacity"
    >
    ...
    <div class="bg-white rounded-2xl shadow-2xl p-6 pt-2 w-80">
      ...
      <p
        v-if="errors.email"
        class="text-red-600 text-[11px] leading-none font-bold mb-4"
      >
        {{ emailErrorMessage }}
      </p>

      <p class="text-gray-800 font-bold flex justify-start">
        Nom d'utilisateur :
      </p>
      <input
      ...
      <p
        ...
        class="text-red-600 text-xs leading-none font-bold mb-4"
      >
        Ne respecte pas les critères de sécurité
      </p>
      ...
      <div class="flex justify-center mt-4">
        <button
          @click="VerifLogin"
          class="bg-[#64067C] text-white rounded-md px-4 py-2 hover:bg-[#850451] transition-colors"
        >
          S'inscrire
        </button>
      </div>
